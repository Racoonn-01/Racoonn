export const dynamic = 'force-dynamic';

import { NextRequest, NextResponse } from 'next/server';
import { appwriteServer } from '@/lib/appwrite/server';
import { authenticatePartnerRequest } from '@/lib/partner-api/middleware';
import { handleApiError, ApiError } from '@/lib/partner-api/errors/api-error';
import { logPartnerApiRequest } from '@/lib/partner-api/security/audit-log';
import { assertPropertyAccess } from '@/lib/partner-api/tenant';

const DATABASE_ID = process.env.APPWRITE_DATABASE_ID || process.env.NEXT_PUBLIC_APPWRITE_DATABASE_ID || '6a3cec630035d63ea963';

export async function GET(
  req: NextRequest,
  { params }: { params: Promise<{ roomId: string }> }
) {
  const startTime = Date.now();
  let partnerCtx;
  let reqId = 'req_init';

  try {
    const auth = await authenticatePartnerRequest(req, 'rooms:read');
    partnerCtx = auth.partner;
    reqId = auth.requestId;

    const { roomId } = await params;

    if (roomId === 'TEST_ROOM_001') {
      return NextResponse.json({
        success: true,
        data: {
          id: 'TEST_ROOM_001',
          propertyId: 'TEST_PROPERTY_001',
          name: 'Sandbox Deluxe Suite',
          type: 'Deluxe',
          status: 'active',
          totalInventory: 10,
          maxOccupancy: 3,
          size: 350,
          basePrice: 4500,
          currency: 'INR',
          mealPlan: 'Breakfast Included',
        },
      }, { headers: { 'X-Request-ID': reqId } });
    }

    let roomDoc;
    try {
      roomDoc = await appwriteServer.databases.getDocument(
        DATABASE_ID,
        'rooms',
        roomId
      );
    } catch {
      throw new ApiError('RESOURCE_NOT_FOUND', `Room '${roomId}' was not found.`, 404);
    }

    if (roomDoc.propertyId) {
      await assertPropertyAccess(partnerCtx.apiKeyId, partnerCtx.partner, roomDoc.propertyId);
    }

    const data = {
      id: roomDoc.$id,
      propertyId: roomDoc.propertyId,
      name: roomDoc.name,
      type: roomDoc.type || 'Standard',
      status: 'active',
      totalInventory: roomDoc.size ? Math.max(1, Math.floor(roomDoc.size / 30)) : 5,
      maxOccupancy: roomDoc.occupancy || 2,
      size: roomDoc.size || null,
      basePrice: roomDoc.discountPrice && roomDoc.discountPrice > 0 ? roomDoc.discountPrice : roomDoc.price,
      currency: 'INR',
      mealPlan: roomDoc.mealPlan || null,
      cancellationPolicy: roomDoc.cancellation || null,
      description: roomDoc.description || null,
      createdAt: roomDoc.$createdAt,
    };

    logPartnerApiRequest({
      partner: partnerCtx.partner,
      apiKeyId: partnerCtx.apiKeyId,
      method: 'GET',
      path: `/api/v1/rooms/${roomId}`,
      statusCode: 200,
      requestId: reqId,
      responseTime: Date.now() - startTime,
      environment: partnerCtx.environment,
    });

    return NextResponse.json({ success: true, data }, { headers: { 'X-Request-ID': reqId } });
  } catch (err) {
    if (partnerCtx) {
      logPartnerApiRequest({
        partner: partnerCtx.partner,
        apiKeyId: partnerCtx.apiKeyId,
        method: 'GET',
        path: `/api/v1/rooms/unknown`,
        statusCode: (err as { statusCode?: number }).statusCode || 500,
        requestId: reqId,
        responseTime: Date.now() - startTime,
        environment: partnerCtx.environment,
      });
    }
    return handleApiError(err, reqId);
  }
}
