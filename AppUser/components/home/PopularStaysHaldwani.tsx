...
export default function PopularStaysHaldwani() {
  ...
  useEffect(() => {
    async function loadHaldwaniStays() {
      try {
        const docs = await getProperties();
        if (docs && docs.length > 0) {
          const haldwaniDocs = docs.filter((d: any) => {
            if (!isActiveProperty(d)) return false;
            const loc = String(d.location || `${d.city || ''} ${d.state || ''}`).toLowerCase();
            return loc.includes('haldwani') || loc.includes('kathgodam');
          });

          if (haldwaniDocs.length > 0) {
            const mapped = haldwaniDocs.map((d: any) => {
              const rawPrice = Number(d.price || d.startingPrice || d.minPrice || d.basePrice || d.pricePerNight || 3500);
              const photos = Array.isArray(d.photos) ? d.photos : [];
              const photoUrl = photos[0] ? String(photos[0]) : 'https://images.unsplash.com/photo-1566073771259-6a8506099945?q=80&w=800&auto=format&fit=crop';

              return {
                id: String(d.$id),
                title: String(d.propertyName || d.title || 'Haldwani Stay'),
                location: String(d.location || d.city || 'Haldwani'),
                details: String(d.description || 'Premium stay in Haldwani'),
                price: `₹${rawPrice.toLocaleString('en-IN')}`,
                rating: d.rating ? String(d.rating) : '4.8',
                reviews: Number(d.reviewsCount || 48),
                image: photoUrl,
              };
            });
            setStays(mapped);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <View>
          <Text style={styles.title}>Popular Stays in Haldwani</Text>
          <Text style={styles.subtitle}>Handpicked luxury resorts & gateway stays</Text>
        </View>
      </View>
