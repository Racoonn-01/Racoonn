...
export default function PopularStaysNainital() {
  ...
  useEffect(() => {
    async function loadNainitalStays() {
      try {
        const docs = await getProperties();
        if (docs && docs.length > 0) {
          const nainitalDocs = docs.filter((d: any) => {
            if (!isActiveProperty(d)) return false;
            const loc = String(d.location || `${d.city || ''} ${d.state || ''}`).toLowerCase();
            return loc.includes('nainital');
          });

          const mapped = nainitalDocs.map((d: any) => {
            const rawPrice = Number(d.price || d.startingPrice || d.minPrice || d.basePrice || d.pricePerNight || 4500);
            const photos = Array.isArray(d.photos) ? d.photos : [];
            const photoUrl = photos[0] ? String(photos[0]) : 'https://images.unsplash.com/photo-1510798831971-661eb04b3739?q=80&w=800&auto=format&fit=crop';

            return {
              id: String(d.$id),
              title: String(d.propertyName || d.title || 'Nainital Stay'),
              location: String(d.location || d.city || 'Nainital'),
              details: String(d.description || 'Heritage lakeside resort stay'),
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <View>
          <Text style={styles.title}>Popular Stays in Nainital</Text>
          <Text style={styles.subtitle}>Lakeside heritage stays & pine hill view resorts</Text>
        </View>
      </View>
