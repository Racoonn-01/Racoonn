const { Client, Databases } = require('node-appwrite');
const client = new Client()
    .setEndpoint('https://sgp.cloud.appwrite.io/v1')
    .setProject('6a3bce6900381359c3ce')
    .setKey('standard_');
const databases = new Databases(client);

const dbId = '6a3cec630035d63ea963'; 

async function run() {
    const res = await databases.listCollections(dbId);
    const vendorCol = res.collections.find(c => c.name === 'Vendors');
    if (!vendorCol) {
        console.error("Vendors collection not found!");
        return;
    }
    const colId = vendorCol.$id;
    
    try {
        await databases.createBooleanAttribute(dbId, colId, 'allow24PercentGst', false, false);
        console.log(`Created boolean attr allow24PercentGst`);
    } catch (e) {
        console.log(`Skipped allow24PercentGst: ${e.message}`);
    }
    
    console.log("Done");
}
run();
